...
if(isset($_POST['ID']) && isset($_POST['password']) && isset($_POST['password_CK']) && isset($_POST['nickname']) && isset($_POST['user_name']) && isset($_POST['user_email']) && isset($_POST['user_phone']) && isset($_POST['gender'])){
    ...
    //주소창 get
    $user_info = "ID=".$ID."&password=".$password."&password_CK=".$password_CK."&nickname=".$nickname."&user_name=".$user_name."&user_email=".$user_email."&user_phone=".$user_phone."&gender=".$gender;
    ...
    if(empty($ID)){//ID 대한 에러 체크    
        ...
        if(mysqli_num_rows($order_ID)>0){ //만약 아이디가 이미 존재한다면
        ...
        else if(mysqli_num_rows($order_ID)<0){ //만약 아이디가 중복된 것이 없다면
            ...
            $sql_save = "insert into member(ID, password, nickname, user_name, user_email, user_phone, gender, size, tone) values('$ID','$password','$nickname', '$user_name', '$user_email', '$user_phone', '$gender', '$size', '$tone')";
